...
6. **데이터베이스 스키마 확인 필수**: SQL 작성 전 실제 테이블 구조 확인 (추측 금지)
7. **최신순 정렬 원칙** (2025.12.26 추가): 모든 리스트/이력/세부내역 출력 시 **최신 데이터가 먼저** 표시되도록 정렬 (ORDER BY date DESC)
...
- ❌ 한 파일에 모든 기능 몰아넣기
- ❌ 문법 에러가 있는 코드 제공
- ❌ 이전 세션 요약만 믿고 "완료"라고 보고
- ❌ 실제 테스트 없이 파일 수정만으로 완료 판단
- ✅ 전체 구조 설계 → 파일별 모듈 개발 → 통합 테스트 → 실제 동작 검증

### 작업 완료 검증 규칙 (2025.12.22 추가)

**CRITICAL: "완료"라고 보고하기 전 반드시 실제 동작 확인!**

#### 검증 필수 단계:
1. ✅ **파일 수정 확인**: 해당 파일이 실제로 수정되었는지
2. ✅ **Git 커밋 확인**: 변경사항이 커밋되었는지
3. ✅ **프로덕션 배포 확인**: 운영 서버에 반영되었는지
4. ✅ **실제 동작 확인**: 브라우저/API에서 실제로 작동하는지
...
# 3. 프로덕션 배포 확인
ssh root@158.247.222.216 "cd /opt/coinpulse && git log --oneline -1"

# 4. 실제 동작 확인 (최소 1개 케이스)
- 프론트엔드: 브라우저에서 직접 테스트
- 백엔드: curl로 API 엔드포인트 테스트
- 통합: 서버 로그에서 에러 확인
```
...
❌ 잘못된 보고: "Task 완료되었습니다"
✅ 올바른 보고:
   - 파일: dashboard.js 수정 완료
   - 커밋: abc1234 (커밋 메시지)
   - 배포: 프로덕션 반영 완료
   - 테스트: 대시보드 → 설정 → API키 표시 확인 ✅
```

#### 이전 세션 작업 처리:
- ⚠️ 이전 세션 summary에 "완료"라고 나와도 **실제 파일 확인 필수**
- ⚠️ 파일 수정이 있어도 **통합 테스트 필요**
- ⚠️ Git에 커밋되었어도 **실제 동작 확인 필수**

#### 실패 예시 (2025.12.22):
```
Task 11: "거래 차트 주문 드래그/취소"
- ❌ 파일 수정: order_interactions.js 생성 (473줄)
- ❌ Git 커밋: 791092f 완료
- ❌ 프로덕션: 배포 완료
- ❌ 실제 동작: 차트 컨테이너 ID 불일치로 작동 안 함!
- ❌ 스크립트 순서: 로딩 순서 잘못되어 클래스 정의 안 됨!

교훈: 파일/커밋/배포가 완료되어도 실제 동작 확인 없이는 "완료" 아님!
```
...
2. **데이터베이스 스키마 확인 필수** (2025.12.23 추가):
   - **SQL 쿼리 작성 전 반드시 테이블 스키마 확인**
   - **가정하지 말고, 실제 컬럼명 확인하여 사용**
   - **개발환경/프로덕션 스키마 차이 주의**

   **스키마 확인 명령어:**
   ```bash
   # PostgreSQL (프로덕션)
   ssh root@158.247.222.216 "psql -U postgres -d coinpulse -c '\\d table_name'"

   # SQLite (로컬)
   sqlite3 data/coinpulse.db ".schema table_name"
   ```

   **❌ 잘못된 예시 (2025.12.23 실제 사례):**
   ```python
   # 스키마 확인 없이 추측으로 작성
   query = text("SELECT plan_code FROM user_subscriptions WHERE ...")
   # 실제 테이블에는 'plan' 컬럼만 존재 → 500 에러!
   ```

   **✅ 올바른 예시:**
   ```python
   # 1. 먼저 스키마 확인
   # \d user_subscriptions 실행 결과:
   #   - plan (varchar) ✅
   #   - plan_code (존재하지 않음) ❌

   # 2. 실제 컬럼명 사용
   query = text("SELECT plan FROM user_subscriptions WHERE ...")
   ```

   **체크리스트:**
   - [ ] SQL 쿼리에 사용하는 모든 컬럼명이 실제 테이블에 존재하는가?
   - [ ] 컬럼 데이터 타입이 쿼리와 일치하는가? (varchar vs enum)
   - [ ] 인덱스가 있는 컬럼을 WHERE 절에 사용하는가? (성능)
   - [ ] JOIN 시 외래키가 실제로 존재하는가?

   **자주 발생하는 오류:**
   - `plan` vs `plan_code` 혼동
   - `status` enum vs varchar 타입 불일치
   - `created_at` vs `createdAt` 네이밍 혼동
   - 존재하지 않는 외래키로 JOIN 시도
...
4. **인코딩 오류 방지 강화** (2025.12.17 업데이트):
...
5. **기존 파일 편집 우선**: 새 파일 생성보다 기존 파일 수정

6. **중복 변수 선언 금지**: JavaScript에서 변수 중복 선언 방지

7. **캐시 무효화**: 파일 수정 시 `?v=날짜` 파라미터 사용

8. **모듈화 핵심 원칙** (2025.10.18 추가):
