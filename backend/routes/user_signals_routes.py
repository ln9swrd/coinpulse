        ...
        with get_db_session() as session:
            ...
            stats_query = text("""
                SELECT
                    ...
                    COUNT(CASE WHEN status = 'win' OR (status IN ('closed', 'expired') AND exit_price > entry_price) THEN 1 END) as wins,
                    COUNT(CASE WHEN status = 'lose' OR (status IN ('closed', 'expired') AND exit_price <= entry_price) THEN 1 END) as losses,
